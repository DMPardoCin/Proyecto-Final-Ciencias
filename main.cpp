...
void inicializarBotones()
{
    ...
    btnInit(btnEnlazar, px, y, bw, bh, "[~] Enlazar nodos");
    y += gap;
    ...
    y += gap;
    btnInit(btnReset, px, y, bw, bh, "[R] Reiniciar sim.");
    y += gap;
...
void actualizarTramas()
{
    for (auto &t : tramas)
    {
        ...
        int pA = t.ruta[t.pos];
        int pB = t.ruta[t.pos + 1];

        // Validar indices contra el vector de elementos
        ...
        t.ticksVida++;
        // 30 ticks por salto -> velocidad constante
        float prog = (float)t.ticksVida / 30.0f;
        if (prog > 1.0f)
            prog = 1.0f;
        ...
        // Avanzar al siguiente salto cuando completa el actual
        if (t.ticksVida >= 30)
        {
            t.pos++;
            t.ticksVida = 0;
...
void dibujarModoActual()
{
    ...
    {
    case MODO_NORMAL:
        texto(px + 4, py + 4, "Modo: Normal (arrastrar)");
        break;
    case MODO_CREAR_PC:
        texto(px + 4, py + 4, "Modo: Clic -> crear PC");
        break;
    case MODO_CREAR_ROUTER:
        texto(px + 4, py + 4, "Modo: Clic -> crear Router");
        break;
    case MODO_CREAR_SWITCH:
        texto(px + 4, py + 4, "Modo: Clic -> crear Switch");
        break;
    case MODO_ENLAZAR:
        if (nodoEnlaceOrigen == -1)
    ...
    case MODO_ELIMINAR_ENLACE:
        if (elimEnlaceA == -1)
            texto(px + 4, py + 4, "Elim.enlace: elige nodo A");
        else
            texto(px + 4, py + 4, "Elim.enlace: elige nodo B");
        break;
...
void dibujarPanelDatos()
{
    ...
    for (auto &t : tramas)
        if (!t.entregada)
            act++;
    texto(px, py, "Tramas activas: " + to_string(act));
    py += 18;
    texto(px, py, "Entregadas:     " + to_string(totalEntregadas));
    py += 18;
    texto(px, py, "Nodos: " + to_string(grafo.numNodos) + "  Enlaces: " + to_string((int)enlacesVis.size()));
    py += 18;
    if (simPausada)
    {
        color_rgb(220, 180, 40);
    ...
    if (nodoSeleccionado >= 0 && nodoSeleccionado < grafo.numNodos)
    {
        ...
        for (auto &e : n.adyacentes)
        {
            if (e.destino < grafo.numNodos && py < AH - 60)
            {
                ...
                texto(px + 6, py, "->" + grafo.nodos[e.destino].nombre + " C:" + to_string(e.costoOSPF) + " " + to_string(e.bandwidthBps / 1000000) + "M");
                py += 16;
            }
        }
        py += 4;
    ...
    {
        color_rgb(90, 90, 90);
        texto(px, py, "Clic en un nodo");
        py += 16;
        texto(px, py, "para ver sus datos");
        py += 24;
    }
    ...
    if (dijkstraOrigen >= 0 && !dijkstraRes.distancia.empty())
    {
        ...
        for (int i = 0; i < grafo.numNodos && py < AH - 40; i++)
        {
            ...
            string dist = dijkstraRes.distancia[i] == INF
                              ? "INF"
                              : to_string(dijkstraRes.distancia[i]);
            ...
            texto(px, py, grafo.nodos[i].nombre + " C:" + dist + " | " + rutaStr);
            py += 16;
        }
    }
    else if (py < AH - 60)
    ...
    texto(px, ly, "Leyenda enlaces:");
    ly += 16;
    color_rgb(60, 200, 60);
    rectangulo_lleno(px, ly + 3, px + 14, ly + 10);
    color_rgb(150, 150, 150);
    texto(px + 18, ly, "100M (C1)");
    color_rgb(220, 200, 50);
    rectangulo_lleno(px + 90, ly + 3, px + 104, ly + 10);
    color_rgb(150, 150, 150);
    texto(px + 108, ly, "10M (C10)");
    ly += 18;
    color_rgb(220, 70, 70);
    rectangulo_lleno(px, ly + 3, px + 14, ly + 10);
    color_rgb(150, 150, 150);
    texto(px + 18, ly, "1M (C100)");
    color_rgb(0, 230, 230);
    rectangulo_lleno(px + 90, ly + 3, px + 104, ly + 10);
    color_rgb(150, 150, 150);
    texto(px + 108, ly, "Ruta activa");
}
...
{
    string prefijo = (tipo == PC) ? "PC" : (tipo == ROUTER) ? "R"
                                                            : "SW";
...
void manejarClicRed()
{
    ...
    if (modoActual == MODO_ENLAZAR)
    {
        ...
        {
            if (nodoEnlaceOrigen == -1)
                nodoEnlaceOrigen = idx;
            else
            {
                crearEnlace(nodoEnlaceOrigen, idx);
                nodoEnlaceOrigen = -1;
                modoActual = MODO_NORMAL;
            }
        }
...
void tickSimulacion()
{
    // Las tramas se animan siempre, incluso con sim pausada
    actualizarTramas();
    // Saturacion y envio automatico solo si simulando
    if (simPausada)
        return;
    tickActual++;
    for (Nodo &n : grafo.nodos)
        n.actualizarSaturacion();
    if (tickActual % 60 == 0 && grafo.numNodos >= 2)
        enviarTrama();
}
...
int main()
{
    ...
    while (true)
    {
        ...
        if (btnClick(btnEnviarTrama))
        {
            enviarTrama();
            simPausada = false; // arrancar animacion automaticamente
        }
